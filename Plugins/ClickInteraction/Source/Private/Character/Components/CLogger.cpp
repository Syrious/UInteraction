...
//	UE_LOG(LogTemp, Warning, TEXT("Writing summary to log %s"), *Summary);
...
void UCLogger::StartEvent(TSharedPtr<FOwlNode> Event)
{
	...
	StartTimeOfEvents.Add(Event, StartTime);
}
...
void UCLogger::FinishEvent(TSharedPtr<FOwlNode> Event)
{
	...
	for (auto& Property : Properties) {
		...
		if (Subject.Name.Equals("taskContext")) { // TODO hardcoded string
			ActionDone = Property.Value;

			if (ActionDone.Contains("Stack Checking")) bIsStackingEvent = true;
		}
		else if (Subject.Name.Equals("inContact")) {
